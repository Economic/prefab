...
```{r use_example, display_code = "use_theme(claude_r_analysis())"}
withr::with_tempdir({
  file.create(".here")
  app <- cli::start_app(output = "stdout", .auto_close = FALSE)
...
```{r compose, display_code = 'theme <- r_targets() + claude_r_targets()\n\ncreate_project("my-project", theme)'}
withr::with_tempdir({
  app <- cli::start_app(output = "stdout", .auto_close = FALSE)
  theme <- r_targets() + claude_r_targets()
  create_project(tempfile("my-project"), theme)
  cli::stop_app(app)
})
```
...
prefab ships with some basic themes, but its real goal is to make it simple for you to apply the kinds of project scaffolding that you prefer. There are three ways to build your own themes:

**From a directory of files.** Arrange template files in a folder and `theme_from_dir()` turns them into a theme.
...
**Ship in a package.** This package ships with several basic themes, and you can add themes (which are just functions) to your own package. Use `from_package()` to resolve template files from `inst/`. Best for sharing themes across an organization.

More information on building and deploying themes is in the [Getting Started vignette](https://economic.github.io/prefab/articles/prefab.html).
